...
def find_path(
    ...
    if col_checker is None or isinstance(
        col_checker, workspace_graph.Edge_Checker
    ):  ######################################################### edge checker True or False이므로 True
    ...
    path = o.find_path(
        init_pos, time_limit=time_limit
    )  ###########################################################################33 astar path 각 에이전트마다 나옴 find_path는 아래 Od_Mstar안의 함수
    num_nodes = o.get_memory_useage(False)  # 62
    corrected_mem = o.get_memory_useage(True)  # 70
    if get_obj:
        return path, o
    if return_memory:
...
class Od_Mstar(object):
    ...
    def __init__(
        ...
        if self.col_checker is None:
            ...
            )  #############################################33333333333333333333 edge checker
        ...
        self.gen_policy_planners(
            sub_search, self.obs_map, self.goals
        )  ################################################################# generate Astar policy 아래
    ...
    def find_path(self, init_pos, time_limit=5 * 60):
        ...
        open_list = SortedCollection.SortedCollection(
            init_nodes, key=self.open_list_key
        )  ################################################################ sortedCollection
    ...
    def od_mstar_neighbors(self, node):
        ...
        if (len(node.col_set) > 0 and rob_dex in node.col_set[0]) or self.full_space:
            ...
            neighbors = [
                self.sub_search[self.policy_keys[rob_dex]].get_step(
                    start_coord[rob_dex]
                )
            ]
        ...
        for i in neighbors:
            ...
            temp_cost = self.od_mstar_transition_cost(
                start_coord, node.cost, i, rob_dex
            )
    ...
    def get_subplanner_keys(self, col_set):
        ...
        global_col = list(
            map(lambda y: tuple(map(lambda x: self.rob_id[x], y)), col_set)
        )

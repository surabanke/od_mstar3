...
def Astar_Graph(
    world_descriptor, goal=None, connect_8=False, diagonal_cost=False, makespan=False
):
    ...
    return Astar_Policy(
        ...
        lambda x: GridGraphConn4WaitAtGoal(
            x, goal, wait_cost=1.0, diagonal_cost=diagonal_cost
        ),
        goal,
...
class Priority_Graph(interface.Policy_Interface):
    ...
    def get_neighbors(self, coord):
        ...
        return map(
            lambda x: (x[0], x[1], min(self.max_t, coord[-1] + 1)), pos_neighbors
        )
...
class Edge_Checker(interface.Planner_Edge_Checker):
    ...
    def simple_incremental_cross_over(self, c1, c2):
        ...
        for i in range(len(c1) - 1):
            ...
            # opposite
            if disp[0] == -(c2[i][0] - c2[-1][0]) and disp[1] == -(
                c2[i][1] - c2[-1][1]
            ):
                return True
        return False
    ...
    def prio_col_check(
        self, coord, pcoord, t, paths=None, conn_8=False, recursive=False
    ):
